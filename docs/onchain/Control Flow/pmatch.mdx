...
`pmatch` is takeing a struct `Term` and returnring a Typescript object with all the names of possible constructors that struct has, based on its definition. 
...
Fortunately for us, if the struct definition has only one possible constructor, this struct term directly exposes the `extract` method 
