...
If this seems familiar it's because it works on the same principle of `pList` we saw in the explanation of [simple values](../Simple%20Values).
...
Fortunately for us there are some [builtin functions](./builtins) that form the fundamentals of the language.
...
Here we are using the `peqInt` builtin function; the `$` method is a short form for the [`papp` function](./Function_appliaciton) and is how we pass arguments to a `plu-ts` function (we'll cover function application in the very next section).
