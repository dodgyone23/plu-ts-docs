...
This characteristic of having multiple constructors will allow for the creation of *custom control flows* through the use of [`pmatch` described in its own section](../../Control%20Flow/pmatch).
...
Like [aliases](../aliases), structs also do have custom `plu-ts` types; which can be accessed using the `type` static property
